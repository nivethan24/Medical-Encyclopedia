...
def getAllItems(allAnswers, extractingIds, extractingItems, keyName):
    returnExtractItems = {}
...
def extractData(symptom, allAnswers, allDates, diseasesList, drugsList, symptomsList):
    extractingIds = []
    print(symptom)
    for id in range(len(allAnswers)):
        if symptom in allAnswers[id]:
    ...
    if (len(extractingIds) != 0):
        ...
        returnValues['diseasesPresent'] = getAllItems(allAnswers, extractingIds, diseasesList, 'disease')
        returnValues['symptomsPresent'] = getAllItems(allAnswers, extractingIds, symptomsList, 'symptom')
        returnValues['drugsPresent'] = getAllItems(allAnswers, extractingIds, drugsList, 'drug')
